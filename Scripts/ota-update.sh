...
do
	if [[ "$(file -b $file)" != "data" && "$(file -b $file)" != "directory" ]]; then
        	...
        	rm -rf /firmware/crypted/ > /dev/null 2>&1
		exit 0
...
mount -i -t ecryptfs /firmware/crypted/ /firmware/crypted/ -o ecryptfs_sig=faa2eb740f201c8d,ecryptfs_fnek_sig=5ac0db675b79d628,ecryptfs_cipher=aes,ecryptfs_key_bytes=16,ecryptfs_unlink_sigs > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
if [ -f "/firmware/crypted/authorised" ]; then
	if [ $(cat /firmware/crypted/authorised) -eq 0 ]; then
		echo "Authorisation failed"
                echo FW_UPD_ERR_AUTHENTICATION_FAILED > /etc/firmware-update-status
		sync
		umount /firmware/crypted > /dev/null 2>&1
		rm -rf /firmware/crypted/ > /dev/null 2>&1
        	exit 0
	else
		echo "Authorised"
		echo 0 > /firmware/crypted/authorised
	fi
else
        echo FW_UPD_ERR_AUTHENTICATION_FAILED > /etc/firmware-update-status
	sync
...
new_kernel=$(ls /firmware/crypted | grep vmlinuz)
new_initrd=$(ls /firmware/crypted | grep initrd)

#cur_app=$(basename /home/appusr_swilch/swilch/swilch_app*)
#new_app=$(basename /firmware/crypted/swilch_app*)

cur_app=$(ls /home/appusr_swilch/swilch | grep swilch_application)
new_app=$(ls /firmware/crypted | grep swilch_application)
...
if [ -f "/firmware/crypted/app_update" ]; then
        if [ $(cat /firmware/crypted/app_update) -eq 1 ]; then
                echo "App update enabled"

		if [ ! -z $new_app ]; then
			echo 0 > /firmware/crypted/app_update
...
if [[ $app_update -eq 1 && $cont -eq 1 ]]; then
        ...
        cp -a /firmware/crypted/$new_app /home/appusr_swilch/swilch/$new_app > /dev/null 2>&1
...
if [[ $kernel_update -eq 1 && $cont -eq 1 ]]; then
        ...
        cp /firmware/crypted/$new_kernel /boot/$new_kernel
        cp /firmware/crypted/$new_initrd /boot/$new_initrd
